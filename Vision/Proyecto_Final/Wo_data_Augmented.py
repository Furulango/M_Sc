    ...
    if df is not None:
        ...
        if X is not None and y is not None:
            ...
            mlp_model = MLPClassifier(hidden_layer_sizes=(100, 50),
                                     ...
                                     max_iter=1000,
                                     random_state=42)
            mlp_model, _, mlp_acc, mlp_cv, mlp_report = train_evaluate_model(
                X, y, label_map, feature_cols, mlp_model, "MLP", output_path)
            model_results.append({
                'name': 'MLP',
                'model': mlp_model,
                'accuracy': mlp_acc,
            ...
            knn_model, _, knn_acc, knn_cv, knn_report = train_evaluate_model(
                X, y, label_map, feature_cols, knn_model, "KNN", output_path)
            model_results.append({
                'name': 'KNN',
                'model': knn_model,
                'accuracy': knn_acc,
