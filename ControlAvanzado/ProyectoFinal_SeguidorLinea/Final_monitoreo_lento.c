...
int sendCounter = 0;     // Contador para controlar frecuencia de envío
int sendFreq = 10;       // Enviar cada 10 ciclos (ajustable)
...
void init(void){
    ...
    uart_println("F1 = Envio rapido");
    uart_println("F2 = Envio medio");
    uart_println("F3 = Envio lento");
    uart_println("===============================");
}
    ...
    while(uart_kb_hit()){
        ...
        if(ch == 'X'){
            ...
            if(cmdIndex == 2){
                ...
                if(cmd[0] == 'M' && cmd[1] >= '0' && cmd[1] <= '3'){
                    monitorMode = cmd[1] - '0';
                    uart_println(">>> Modo cambiado");
                }
                else if(cmd[0] == 'F' && cmd[1] >= '1' && cmd[1] <= '3'){
                    if(cmd[1] == '1') sendFreq = 5;      // Rápido
                    else if(cmd[1] == '2') sendFreq = 15; // Medio
                    else if(cmd[1] == '3') sendFreq = 30; // Lento
                    uart_println(">>> Velocidad cambiada");
                }
                cmdIndex = 0;
            }
        }
...
void main(void){
    ...
    for(;;){
        ...
        if(sFlg){
            ...
            // Solo enviar datos cada 'sendFreq' ciclos
            if(sendCounter >= sendFreq){
                sendCounter = 0;
                sendTelemetry();
            }
        }
